...
from notification_db import (
    ...
    check_and_create_notifications, delete_notification, delete_all_notifications
)
...
async def delete_notification_endpoint(request: Request, notification_id: int):
    user = require_auth(request)
    success = delete_notification(notification_id, user["user_id"])
...
async def delete_all_notifications_endpoint(request: Request):
    user = require_auth(request)
    delete_all_notifications(user["user_id"])
    return RedirectResponse(url="/notifications", status_code=status.HTTP_303_SEE_OTHER)
